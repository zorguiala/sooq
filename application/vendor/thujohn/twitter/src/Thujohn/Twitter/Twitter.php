...
class Twitter extends tmhOAuth {
	...
	public function __construct(Config $config, SessionStore $session)
	{
		...
		// Get user autoshare info
		$autoshare = DB::table('auto_share')->where('user_id', $user_id)->first();

		$this->parent_config                    = [];
		$this->parent_config['consumer_key']    = $autoshare->tw_consumer_key;
		$this->parent_config['consumer_secret'] = $autoshare->tw_consumer_secret;
		$this->parent_config['token']           = $autoshare->tw_access_token;
		$this->parent_config['secret']          = $autoshare->tw_access_token_secret;
